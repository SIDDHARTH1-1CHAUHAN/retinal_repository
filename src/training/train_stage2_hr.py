...
from src.models.stage2_hr_vit import HR_CLASS_NAMES, HR_TASK_SPEC, build_stage2_hr_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Stage 2 HR Vision Transformer.")
...
def resolve_settings(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "run_dir": args.run_dir or task.get("run_dir", "reports/stage2_hr"),
        "figures_dir": args.figures_dir or task.get("figures_dir", "reports/figures/stage2_hr"),
...
def run_evaluation(
    ...
    plot_confusion_matrix(
        ...
        class_names=list(HR_CLASS_NAMES),
        output_path=figures_dir / f"{split_name}_confusion_matrix.png",
        title=f"Stage 2 HR {split_name.title()} Confusion Matrix",
    )
    plot_prediction_examples(
        image_paths=frame["image_path"].tolist(),
        true_labels=[HR_CLASS_NAMES[index] for index in labels],
        predicted_labels=[HR_CLASS_NAMES[index] for index in predictions],
        confidences=probabilities.max(axis=1).tolist(),
        output_path=figures_dir / f"{split_name}_prediction_examples.png",
        title=f"Stage 2 HR {split_name.title()} Predictions",
    )
...
def main() -> None:
    ...
    model = build_stage2_hr_model(model_config)
    model.compile(
    ...
    plot_training_history(history_payload, figures_dir, prefix="stage2_hr")
